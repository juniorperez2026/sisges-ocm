...
@Injectable()
export class SqlServerConnectionService implements OnApplicationShutdown {
  private readonly logger = new Logger(SqlServerConnectionService.name);
  ...
  constructor(private readonly configService: ConfigService) {
    this.enabled = this.configService.getOrThrow<boolean>('SQL_SERVER_ENABLED');
  ...
  async getPool(): Promise<ConnectionPool> {
    if (!this.enabled || !this.configuration) {
      throw new Error('SQL Server integration is disabled');
    }
    ...
    this.connectionPromise = candidatePool
      ...
      .then((connectedPool) => {
        ...
        this.logger.log({
          event: 'sql_server_pool_connected',
          database: this.configuration?.database,
          poolMax: this.configuration?.pool?.max,
          poolMin: this.configuration?.pool?.min,
        });
  ...
  async onApplicationShutdown(): Promise<void> {
    ...
    this.logger.log({
      event: 'sql_server_pool_closed',
    });
  }

  private createConfiguration(): SqlServerConfig {
    return {
      server: this.configService.getOrThrow<string>('SQL_SERVER_HOST'),

      port: this.configService.getOrThrow<number>('SQL_SERVER_PORT'),

      database: this.configService.getOrThrow<string>('SQL_SERVER_DATABASE'),

      user: this.configService.getOrThrow<string>('SQL_SERVER_USERNAME'),

      password: this.configService.getOrThrow<string>('SQL_SERVER_PASSWORD'),
      ...
      pool: {
        max: this.configService.getOrThrow<number>('SQL_SERVER_POOL_MAX'),

        min: this.configService.getOrThrow<number>('SQL_SERVER_POOL_MIN'),
      ...
      options: {
        encrypt: this.configService.getOrThrow<boolean>('SQL_SERVER_ENCRYPT'),
